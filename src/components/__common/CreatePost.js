			...
			>
				<div className="create_post__body">
					<TextField
						...
						name="title"
						ref={ ref=>{formData.InputTitle = ref} }
					/>
					<Editor
		            	hashtag={{}}
					...
					<div className="real_time_preview">
						...
						<div dangerouslySetInnerHTML={{__html: editorContents[0] && draftToHtml(convertToRaw(editorContents[0].getCurrentContent()))}}></div>
					</div>
				</div>
			</Dialog>
		</div>
...
export default class CreatePost extends React.Component {
	...
	handleChangeSelectTag = (tagSelected) => {
		...
		if(tagSelected.length <= maxSelectedOptions) {
			this.setState(state => {state.dataTag.value = tagSelected})
		}
	}

	handleSelectReward = (event, index, value) => {
		this.setState(state => {state.valueReward = value})
	}
	...
	handleEditorStateChange = (index) => (editorContent) => {
		...
		this.setState( state => { state.editorContents = editorContents	})
	}
	...
	handleCreatePost = () => {
		...
		if (!getRegExp.typeString().test(inputTitle.value)){
			// countError++
			this.setState((state) => { state.formError[inputTitle.name] = "Title is not null." })
		} else {
			this.setState((state) => { state.formError[inputTitle.name] = null })
		}
