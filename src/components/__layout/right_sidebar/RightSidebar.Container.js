...
export default class RightSidebar extends React.Component {
	...
	updatePositionSideBar = () => {
		this.setState(state => { state.isOpenSideBar = window.innerWidth <= 1200 ? false : true })
	}
	...
	handleToggleSideBar = () => {
		...
		this.setState(state => { state.isOpenSideBar = !state.isOpenSideBar })
	}
		...
		return (
			<aside id="right_sidebar" className={this.state.isOpenSideBar ? "-is-open" : ""}>
				...
				<button className="btn_toggle_sidebar" onClick={this.handleToggleSideBar}>
					<i className="material-icons">{this.state.isOpenSideBar ? "keyboard_arrow_right" : "keyboard_arrow_left"}</i>
				</button>
			</aside>
		)
	}
